...
static int counter=0; //counting rounds. Needed to see the final output
...
PROCESS_THREAD(udp_server_process, ev, data)
{
  ...
  printf("SINK: PERIOD: %d\n", PERIOD);
  printf("SINK: DixonQ active n value: %d\n",dixon_n_vals);
  printf("SINK: DixonQ confidence_level: %d\n",confidence_level);
  printf("SINK: PERIOD from project-conf.h: %d\n", PERIOD);
...
/* Mode 3 - derived from link local (MAC) address */
  ...
  while(1) {
    ...
    if(etimer_expired(&periodic)) {
      ...
      if (counter%10 == 0){ /* every ten PERIODS print the round */
      	printf("SINK: Round: %d\n",counter);
      }

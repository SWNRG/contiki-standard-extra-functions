...
static uint8_t queue_index = 0; /* no queue in standard C? */
static int incoming_values[dixon_n_vals];
float critical_value = 0; //confidence level to compare with
 ...
 */
static int addDixonQOut(int value){
	uint8_t answer = 0;
	float q_small;
	float q_big;
...
#if PRINT_ON 
		printf("Reseting queue_index in dixonQtestOut= 0\n");
#endif
		queue_index = 0;	
	...
	if(all_values_exist == 1){ /* start calculating possible outliers */
		...
		sortInt(sort, dixon_n_vals); /* sort a NEW array */
		...
		/* printing float */
		printf("0.%03u\n",(int)(critical_value*1000-(int)critical_value));
#endif			
		/* first confidence for n==3 */
		if(q_small > critical_value){ 
#if PRINT_ON
			printf("Value %d, ", sort[0]);
			printf(" with q_small=");
			printf("0.%03u\n",(int)(q_small*1000-(int)q_small));
			printf(" is bigger than 0.%03u",(int)(critical_value*1000-(int)critical_value));
			int i=0;
			printf(" in array: ");
			for(i=0;i<dixon_n_vals;i++){
				printf("%d ",sort[i]);
